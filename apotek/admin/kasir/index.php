...
<?php $dataselect = mysqli_query($conn, "SELECT * FROM produk WHERE toko='" . $_SESSION['toko'] . "'");
$jsArray = "var nama_produk = new Array();";
$jsArray1 = "var harga_jual_setelah = new Array();";
...
<form method="post">
  <div class="row">
        ...
        <datalist id="datalist1">
          <?php if (mysqli_num_rows($dataselect)) { ?>
            <?php while ($row_brg = mysqli_fetch_array($dataselect)) { ?>
              ...
              $jsArray1 .= "harga_jual_setelah['" . $row_brg['kode_produk'] . "'] = {harga_jual_setelah:'" . addslashes($row_brg['harga_jual_setelah']) . "'};";
    ...
    <div class="col-8 col-sm-4 col-md-4 col-lg-2 mb-3">
      ...
      <input type="number" name="Charga" placeholder="0" id="harga_jual_setelah" onchange="InputSub()" class="form-control form-control-sm bg-light" readonly>
    </div>
...
if (isset($_POST['InputCart'])) {
  ...
  if ($liat > 0) {
    ...
    if ($liatlg > 0) {
      ...
      $tambahdata = mysqli_query($conn, "INSERT INTO cart (invoice,kode_produk,nama_produk,harga,harga_modal,qty,subtotal,toko)
       values('$inv_c','$Input1','$Input2','$Input3','$hrg_m','$ii','$Input5','" . $_SESSION['toko'] . "')");
      if ($tambahdata) {
        echo '<script>window.location="index.php"</script>';
      } else {
        echo '<script>window.location="index.php"</script>';
    ...
    $bikincart = mysqli_query($conn, "INSERT INTO inv (invoice,pembayaran,kembalian,status,toko) values('$oi','','','proses','" . $_SESSION['toko'] . "')");
    if ($bikincart) {
...
if (isset($_POST['import'])) {
  ...
  while ($ds = mysqli_fetch_array($datacart)) {

    $selSto = mysqli_query($conn, "SELECT * FROM produk WHERE kode_produk ='" . $ds['kode_produk'] . "' AND toko='" . $_SESSION['toko'] . "'");
    $sto = mysqli_fetch_array($selSto);
    $stok = $sto['stok'];
    $sisa = $stok - $ds['qty'];
    if ($stok < $ds['qty']) {
      echo '<script>alert("Stok Barang Kurang");history.go(-1);</script>';
    } else {
      $upstok = mysqli_query($conn, "UPDATE produk SET stok='$sisa' WHERE kode_produk='" . $ds['kode_produk'] . "' AND toko='" . $_SESSION['toko'] . "'");
    }
    ...
    if ($Ikembalian >= 0) {
     ...
     SELECT invoice,kode_produk,harga,harga_modal,qty,subtotal,'" . $_SESSION['toko'] . "','" . $_SESSION['username'] . "' FROM cart") or die(mysqli_connect_error());
      ...
      echo '<script>alert("Uang Kurang Dari Total Bayar");</script>';
    }
  }
};
?>
...
<div class="modal fade" id="Exit" tabindex="-1" role="dialog">
  ...
  <script type="text/javascript">
    ...
    function changeValue(kode_produk) {
      ...
      document.getElementById("harga_jual_setelah").value = harga_jual_setelah[kode_produk].harga_jual_setelah;
      document.getElementById("harga_modal").value = harga_modal[kode_produk].harga_modal;
    };

    function InputSub() {
      var harga_jual_setelah = parseInt(document.getElementById('harga_jual_setelah').value);
      var jumlah_beli = parseInt(document.getElementById('Iqty').value);
      var jumlah_harga = harga_jual_setelah * jumlah_beli;
